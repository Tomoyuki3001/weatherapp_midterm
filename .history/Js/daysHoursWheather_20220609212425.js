...
function showWheatherData(data) {
  ...
  data.list.forEach((value) => {
    let dtText = value.dt_txt;
    let dateLetters = dtText.slice(0, 10);
    let strings = value.dt_txt.slice(0, 10);
    if ((dateObject[strings] = dateLetters)) {
      newArray.push(data.list[0]);
      dateObject[strings] = newArray;
    }
  });
  console.log(dateObject[strings]);
}
