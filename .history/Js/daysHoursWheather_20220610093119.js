...
function showWheatherData(data) {
  ...
  let test = data.list[0];
  console.log(data.list);

  data.list.forEach((value) => {
    let date = value.dt_txt.slice(0, 10);
    let time = value.dt_txt.slice(10);
    value.dt_date = date;
    value.dt_time = time;
    delete data.list;
    data.newlist = value;
    let newData = data;
    // console.log(newData.newlist.dt_date);

    let propertyName = value.dt_date;
    console.log(dateObject[propertyName]);
    dateObject[propertyName] = newArray;
    // if (dateObject.hasOwnProperty(propertyName) == true) {
    //   let result = data.newlist.indexOf("2022-06-10");
    //   console.log(result);
    // }
  });

  // let result = value.some(function (element) {
  //   return element === strings;
  // });
  // console.log(result);
  // if (dateObject[propertyName] === value.dt_txt.slice(0, 10)) {
  //   newArray.push(value);
  // }
