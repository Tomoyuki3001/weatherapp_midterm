...
function showWheatherData(data) {
  ...
  for (let i = 0; i < newDaysArray.length; i++) {
    ...
    <div class="days_day">${moment(newDaysArray[i].dt * 1000).format(
      ...
      />
      <div class="days_temp">${newTemp}&#176C</div>
      <div>Feels like</div>
      <div class="days_temp_feels">${newTempFeels}&#176C</div>
  ...
  for (let i = 0; i < 8; i++) {
    ...
    hoursCard.innerHTML = `
      <img
        ...
        class="weather-icon"
      />
      <div class="temp">Temperature ${newTempHours}&#176C</div>
      <div>Feels like</div>
      <div class="days_temp_feels">${newTempFeelsHours}&#176C</div>
      <div class="days_pop"><span class="iconify days_pop_icon" data-icon="bi:cloud-rain-heavy" style="color: white; font-size: 17px;"></span>${popHours}%</div>
    `;
